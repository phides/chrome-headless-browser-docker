...
Standalone mode:
```
docker run -it --name chrome-headless-standalone --shm-size=1024m --cap-add=SYS_ADMIN \
...
docker run -it --name node-chrome-headless --link hub:hub --cap-add=SYS_ADMIN  phide/chrome-headless-browser-docker -role node -hub http://hub:4444/grid/register -nodeConfig /opt/selenium/config.json
```
